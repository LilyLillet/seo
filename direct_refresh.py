...
class metrica:
    ...
    key_path = 'C:\\Users\\Krugl\\OneDrive\\Рабочий стол\\token.json'
    # key_path = 'C:\\Users\\kalmukds\\NOTEBOOKs\\projects\\keys\\creds_metrics.json'
    vipassana_id = '88227360'
    ...
    def __init__(self, vipassana_id='88227360'):
        # Оптределяем аккаунт откуда бер1м данные
        self.vipassana_id = vipassana_id

    def date_pairs(self, date1, date2, step=1):

        pairs = []
        while date2 >= date1:
            prev_date = date2 - datetime.timedelta(days=step - 1) if date2 - datetime.timedelta(
                days=step) >= date1 else date1
    ...
    def requests(self, date1, date2, metrics, dimensions, filters='', group='day'):
        ...
        base_url += f'&id={metrica.vipassana_id}'
